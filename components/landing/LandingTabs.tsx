...
export default function LandingTabs() {
  ...
  return (
    <section className="surface-card rounded-[28px] p-6 md:p-8">
      <h2 className="text-2xl font-semibold text-slate-900">Everything you need to grow online.</h2>
        ...
        {(Object.keys(tabContent) as TabKey[]).map((tab) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
              activeTab === tab ? "bg-brand-gradient text-white" : "border border-border text-muted hover:bg-border hover:text-slate-900"
            }`}
          >
            {tab}
          </button>
