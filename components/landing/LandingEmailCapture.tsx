...
export default function LandingEmailCapture() {
  ...
  return (
    <section id="email-capture" className="surface-card p-6 md:p-8">
      <h2 className="text-2xl font-semibold text-slate-900">Get product updates</h2>
      ...
      <form onSubmit={onSubmit} className="mt-5 flex flex-col gap-3 sm:flex-row">
        ...
        <input
          ...
          className="h-12 flex-1 rounded-[14px] border border-border bg-primary px-4 text-sm text-slate-900 placeholder:text-muted"
        />
        <button
          type="submit"
      ...
      <p
        className={`mt-3 text-sm transition ${
          status === "success" ? "translate-y-0 opacity-100 text-slate-900" : "translate-y-1 opacity-0"
        }`}
      >
